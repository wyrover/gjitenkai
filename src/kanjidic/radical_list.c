...
void radical_list_init(kanjidic *kanjidic){
  ...
  GtkGrid *grid_radical_list = gtk_builder_get_object(kanjidic->definitions, 
                                                    "grid_radical");
  ...
  for (radical_list;
    ...
    gchar *radical      = ((RadInfo*)radical_list->data)->radical;
    ...
    if(last_strockes_count != strokes_count){
      ...
      gtk_grid_attach(GTK_GRID(grid_radical_list), label_stroke_count, i, j, 1, 1);
    ...
    //add the button
    GtkButton *button_radical = gtk_button_new_with_label(radical);
    g_signal_connect(button_radical, "clicked", on_radical_button_clicked, kanjidic);
    gtk_grid_attach(GTK_GRID(grid_radical_list), button_radical, i, j, 1, 1);
...
void radical_list_update_sensitivity(kanjidic *kanjidic){
  ...
  do{
    ...
    gchar* cur_radical = gtk_button_get_label(button);
